...
const HLSVideoPlayer = forwardRef<HLSVideoPlayerRef, HLSVideoPlayerProps>(
  function HLSVideoPlayer(
      ...
      const initializeHLS = async () => {
        try {
          ...
          if (videoElement.canPlayType('application/vnd.apple.mpegurl')) {
            ...
            videoElement.src = `http://localhost:8000/api/v1/videos/${video.id}/hls/playlist.m3u8`;
            setIsHLSReady(true);
          } else {
            // Fallback to regular MP4
            console.warn('HLS not supported, falling back to MP4');
            videoElement.src = `http://localhost:8000/videos/${video.filename}`;
            setIsHLSReady(true);
          }
        } catch (error) {
    ...
    // Generate frame preview using HLS segments
    const generateFramePreview = useCallback(
      async (time: number) => {
        ...
        try {
          // Try to get frame from HLS thumbnail service
          const response = await fetch(
            `http://localhost:8000/api/v1/videos/${video.id}/hls/thumbnails/${Math.floor(time)}.jpg`
          );
          ...
          if (response.ok) {
            ...
            // Fallback to frame extraction API
      ...
      return (
        <div className='relative bg-black rounded-lg overflow-hidden group aspect-video flex items-center justify-center'>
          <div className='text-center text-white'>
            ...
            <p className='text-gray-300 mb-4'>{hlsError}</p>
            <button
    ...
    return (
      ...
      >
        ...
        {isHLSReady && currentQuality !== 'auto' && (
        ...
        {playbackRate !== 1 && (
          <div className='absolute top-4 right-16 bg-black/80 text-white px-3 py-1 rounded text-sm font-mono'>
            {playbackRate}x
          </div>
        )}
        ...
        >
          <div className='flex items-center gap-3'>
            ...
            {isHLSReady && availableQualities.length > 0 && (
              <div className='relative'>
                <button
                ...
                {showQualityMenu && (
                  <div className='absolute bottom-full mb-2 bg-gray-900 rounded shadow-lg py-1 min-w-[120px]'>
                    {availableQualities.map((quality) => (
                      <button
                        key={quality}
                        onClick={() => {
                          if (ref && 'current' in ref && ref.current) {
                            ref.current.setQuality(quality);
                          }
              ...
              <div className='grid grid-cols-2 gap-4 text-sm'>
                ...
                <div>
                  ...
                  <div className='space-y-1 text-gray-300'>
                    ...
                    <div>
                      <kbd className='bg-gray-800 px-2 py-1 rounded'>G</kbd> Go
                      to timestamp
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className='space-y-1 text-gray-300'>
                    ...
                    <div>
                      <kbd className='bg-gray-800 px-2 py-1 rounded'>M</kbd>{' '}
                      Mute
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className='space-y-1 text-gray-300'>
                    ...
                    <div>
                      <kbd className='bg-gray-800 px-2 py-1 rounded'>4</kbd>{' '}
                      Note
                    </div>
                    <div>
