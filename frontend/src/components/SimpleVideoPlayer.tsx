...
export interface SimpleVideoPlayerRef {
...
const SimpleVideoPlayer = forwardRef<
  SimpleVideoPlayerRef,
...
>(function SimpleVideoPlayer(
  ...
  const [clickCursorPosition, setClickCursorPosition] = useState(0);
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // Don't override current time if we're in the middle of seeking
      if (!isSeeking) {
        setCurrentTime(time);
      }
      onTimeUpdate?.(time);
    ...
    const handleSeeked = () => {
      // Ensure the current time is updated after seeking
      const time = videoElement.currentTime;
      setCurrentTime(time);
      setIsSeeking(false);
    };
  ...
  }, [setCurrentTime, setDuration, setIsPlaying, onTimeUpdate, isSeeking]);
  ...
  const generateFramePreview = useCallback(
    async (time: number) => {
      ...
      try {
        // Try to get frame from backend API
        ...
        if (response.ok) {
          ...
          // Fallback to simple preview with time info
          const previewData = {
            time: time,
            formattedTime: formatTime(time),
            chunkNumber: Math.floor(time / 120) + 1,
            totalChunks: Math.ceil(duration / 120),
          };

          setPreviewImage(JSON.stringify(previewData));
          setPreviewCache((prev) =>
            new Map(prev).set(Math.floor(time), JSON.stringify(previewData))
          );
        }
      } catch (error) {
    ...
    [previewCache, video.id, duration]
  );
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    if (videoRef.current) {
      ...
      // Immediately update the current time to show cursor at clicked position
      setCurrentTime(newTime);
      ...
      try {
        ...
        if (videoRef.current.readyState >= 2) {
          ...
          const clampedTime = Math.max(
            0,
            Math.min(newTime, videoRef.current.duration || duration)
          );

          // Try to ensure video is ready for seeking
          if (videoRef.current.paused && videoRef.current.currentTime === 0) {
            console.log(
              'Video is paused at start, playing briefly to enable seeking...'
            );
            videoRef.current.play();
            setTimeout(() => {
              videoRef.current.pause();
          ...
          // Force update the current time state after a short delay
          setTimeout(() => {
            ...
            if (Math.abs(actualTime - clampedTime) > 1) {
          ...
          console.warn(
            'Video not ready to seek, readyState:',
      ...
      // Show click cursor at the clicked position
      setClickCursorPosition(percentage * 100);
      setShowClickCursor(true);

      // Hide click cursor after 2 seconds
      setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div className='absolute bottom-16 left-0 right-0 px-4'>
        ...
        >
          {/* Playback progress - more visible */}
          <div
            ...
            style={{
              width: `${Math.max(0.5, (currentTime / duration) * 100)}%`,
            }}
          />

          {/* Current time cursor - positioned at the end of progress */}
          <div
            className='absolute top-0 w-2 h-full bg-white border-2 border-blue-500 rounded shadow-lg shadow-blue-500/50 z-10'
            style={{ left: `${(currentTime / duration) * 100}%` }}
          />

          {/* Click cursor - appears after clicking */}
          {showClickCursor && (
            <div
              className='absolute top-0 w-2 h-full bg-red-500 border-2 border-white rounded shadow-lg shadow-red-500/50 animate-pulse z-20'
              style={{ left: `${clickCursorPosition}%` }}
            />
          )}
          ...
          {isHoveringProgress && (
            <div
              className='absolute top-0 w-1 h-full bg-yellow-400 border-2 border-yellow-600 rounded shadow-lg shadow-yellow-400/50 z-15'
              style={{ left: `${hoverPosition}%` }}
            />
          )}

          {/* Annotation markers - small dots only */}
          {annotations.map((annotation) => (
            <div
              key={annotation.id}
              className='absolute top-0 cursor-pointer transition-all pointer-events-auto'
              style={{
        ...
        {isHoveringProgress && (
          <div
            ...
            style={{ left: `${hoverPosition}%` }}
          >
            <div className='bg-black/90 rounded-lg p-3 shadow-lg min-w-[140px]'>
              {isLoadingPreview ? (
                ...
                (() => {
                  // Check if it's a JSON string (fallback data) or image URL
                  if (previewImage.startsWith('{')) {
                    try {
                      const previewData = JSON.parse(previewImage);
                      return (
                        <div className='w-32 h-20 bg-gradient-to-br from-blue-600 to-purple-600 rounded flex flex-col items-center justify-center text-white'>
                          <div className='text-xs font-bold'>
                            {previewData.formattedTime}
                          </div>
                          <div className='text-xs opacity-80'>
                            Chunk {previewData.chunkNumber}/
                            {previewData.totalChunks}
                          </div>
